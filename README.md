...
> ⚠️ **首次部署必读 —— 初始密码不再是固定的 `admin123`**：首次启动 `ensure_default()` 会**自动创建**默认管理员账号 `admin`（admin 角色，数据存 `dbmanager.db`），但**初始密码取决于是否配置**：
> - **未配置** `default_pwd` / `DBM_DEFAULT_PWD` 时：系统自动生成 **16 位高熵随机口令**，并**仅在启动日志打印一次**（`logs/dbmanager.log` 与启动控制台，搜索「初始口令」），**界面不显示、明文不落盘**；
> - **已配置** 时：初始密码取该值（⚠️ 明文写入配置 / 环境变量，存在弱口令风险，建议留空由系统随机生成）。
> 无论哪种，**首次登录强制改密**，改密完成前无法使用业务功能。LAN/公网部署**务必先拿到初始口令并立即改密**，否则内网任意人可在你改密前登录接管。仅在数据库无任何用户且未设 `auth_enabled=1`（`DBM_AUTH=1`）时才为单机无鉴权模式。
...
| 配置项（`[auth]`） | 作用 | 默认 |
|------|------|------|
| `default_pwd` | 首次创建默认 admin 的密码（仅首建生效；⚠️ 明文落盘，**建议留空**——留空则由系统生成 16 位随机口令并打印到启动日志） | 留空（随机生成） |
...
- 默认管理员账号固定为 `admin`；初始密码**未配置时随机生成**（详见上文「首次部署必读」提示，
  启动日志 `logs/dbmanager.log` 搜索「初始口令」获取），**配置 `default_pwd`/`DBM_DEFAULT_PWD` 时取该值** ——
  **首次登录强制改密，生产环境请立即改密**（前端「改密」或 `POST /api/password`）
